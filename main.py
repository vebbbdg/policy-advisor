...
# 注意：Render 免费档下容器看到的客户端 IP 全是内网代理地址（未开代理头信任），
# 故此限额实际是“全站共享池”——有意为之：它把最坏成本锁在约 100 条/小时，
# 宣传期的并发容量也靠它兜底；将来修代理头后会自然退化为真正的每 IP 限额。
CHAT_RATE_LIMIT = os.getenv("CHAT_RATE_LIMIT", "100/hour")
# 访客（未登录）最多可发送的消息条数（终身制），用尽后引导登录（阶段 3.2 获客转化）
GUEST_MESSAGE_LIMIT = int(os.getenv("GUEST_MESSAGE_LIMIT", "5") or 5)
# 注册用户每日消息条数（服务器时区自然日，次日自动恢复）
REGISTERED_DAILY_LIMIT = int(os.getenv("REGISTERED_DAILY_LIMIT", "5") or 5)
...
async def chat_stream(request: Request, data: ChatInput, user: UserContext = Depends(get_current_user)):
    ...
    # 配额（403，与限流 429 区分）：访客终身 GUEST_MESSAGE_LIMIT 条，用尽引导登录；
    # 注册用户每日 REGISTERED_DAILY_LIMIT 条，明天自动恢复。
    if user.is_guest:
        if session_manager.count_user_messages(user.user_id) >= GUEST_MESSAGE_LIMIT:
            raise HTTPException(
                status_code=403,
                detail=f"Guest limit reached ({GUEST_MESSAGE_LIMIT} messages). Please log in to continue.",
            )
    elif session_manager.count_user_messages_today(user.user_id) >= REGISTERED_DAILY_LIMIT:
        raise HTTPException(
            status_code=403,
            detail=f"Daily limit reached ({REGISTERED_DAILY_LIMIT} messages/day). Quota resets tomorrow.",
        )
